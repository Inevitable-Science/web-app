...
export function VestingInitialiser({ children }: { children: React.ReactNode }) {
  ...
  const setVestingContractAddress = useLegacyProjectStore(state => state.setVestingContractAddress);
  const setVestingChainId = useLegacyProjectStore(state => state.setVestingChainId);
  ...
  const vestingContractObj = vestingContracts.find(d => daoData.name.toLowerCase() === d.name);
