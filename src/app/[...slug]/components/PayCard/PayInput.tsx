...
const PayInput = React.forwardRef<HTMLInputElement, PayInputProps>(
  ({ className, inputClassName, label, type, currency, withPayOnSelect, ...props }, ref) => {
    return (
      <div
        className={cn(
          "flex h-30 px-4 py-4 w-full items-center justify-between shadow-sm focus-within:ring-1 focus-within:ring-inset focus-within:ring-[var(--ring)]",
          className
        )}
      >
        <div className="flex flex-col">
          ...
          <input
            type={type}
            className={cn(
              "border-0 bg-transparent pl-0 pr-3 pt-1 pb-0 text-2xl w-full placeholder:text-zinc-400 focus:ring-0 sm:leading-6",
              inputClassName
            )}
            ref={ref}
            placeholder="0.00"
