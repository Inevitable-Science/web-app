...
export function ParticipantsTable() {
  ...
  const [currentData, setCurrentData] = useState<
    ParticipantsQuery["participants"] | null
  >(null);
  ...
  return (
    <div className="mt-2 flex flex-col">
      <h3 className="text-xl">
        {formatTokenSymbol(token?.data?.symbol)} Holders
      </h3>
      {currentData.items.map((participant, index) => {
        const contributionAmount = parseFloat(
          formatUnits(participant.volume, baseToken.decimals)
        );
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <div className="text-grey-100 flex items-center text-xs font-light uppercase">
                {baseToken.isNative ? (
                  "Ξ"
                ) : (
                  <Image
      ...
      <div className="mt-6 flex flex-col items-center gap-2">
        ...
        <p className="text-muted-foreground text-sm font-light">
          Showing {Math.min(pageLimit, totalParticipants)} items out of{" "}
          {totalParticipants}
        </p>
      </div>
    </div>
